...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("Most Retweeted on Empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedMax(max: Tweet): Tweet = left.mostRetweetedMax(right.mostRetweetedMax(if (elem.retweets > max.retweets) elem else max))
